...
def get_total_revenue(account_id=None, start_date=None, end_date=None, db_path=DB_PATH):
    """Total ex-VAT revenue across all ASINs in the period — the denominator for the
    BY-REVENUE overhead split, so a single-SKU view weights overhead the same way /pl
    does (each SKU's share = its revenue ÷ this total)."""
    conn = get_db(db_path)
    where, params = _build_rollup_where(account_id, start_date, end_date)
    row = conn.execute(f"SELECT SUM(sale_price_exvat) AS r FROM pl_line_items {where}", params).fetchone()
    conn.close()
    return (row["r"] or 0.0) if row else 0.0
